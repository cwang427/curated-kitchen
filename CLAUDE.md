...
- **Recipe import Worker → MANUAL (Cloudflare, one-time).** The `worker/`
  ...
  - **root (FREE with Gemini) — paste text or a photo → AI.** Uses Google
    ...
    which shows up as a 404 "no longer available to new users"). On a 503
    "high demand" spike it retries briefly, then auto-falls back to
    `gemini-3.6-flash-lite` (which usually has more free headroom); pinning
    `GEMINI_MODEL` disables that fallback. It reads any layout (blog-style pages the
